...
3. **Compose**: record with the microphone (ASR transcribes as you speak) or type it. Attach a photo with the Spectacles camera. Review, re-listen, delete, or send.
...
- **Snap Cloud (managed Supabase)** — accounts, private addressing, message queue, view-once deletion; photos travel as base64 JPEG in the message row
- **CameraModule** — photo capture on device
...
2. Press play in Preview (or send to Spectacles — camera capture works on device only).
