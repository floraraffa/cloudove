...
The messaging backend runs on **Snap Cloud (managed Supabase)**: user accounts, private addressing, and the view-once message queue. The hackathon Snap Cloud project is bundled so the experience runs out of the box. To point it at your own project instead: Supabase plugin → **Import Credentials** → assign the generated asset to the **CloudMessaging** script's `Supabase Project` input, then run [snap-cloud-setup.sql](snap-cloud-setup.sql) in the SQL Editor.
